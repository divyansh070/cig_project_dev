...
3. **Pre-signed URLs for Media Streaming**: To prevent the backend server from being choked by massive media bandwidth, the server never actually streams large files. Instead, it generates temporary AWS S3 Pre-signed URLs. The client's browser then fetches the uncompressed images and videos directly from Amazon's localized infrastructure, completely decoupling compute from storage bandwidth.
...
3. **Dynamic Watermarking Engine**: To protect intellectual property, standard users cannot download raw images directly. The backend intercepts download requests and dynamically burns a semi-transparent watermark (containing the Event Name and Photographer's Username) directly into the image bytes before serving it. Video assets safely bypass this engine and are streamed directly without modification.
